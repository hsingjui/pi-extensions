...
export function serializeInstructionsToResponsesInput<TApi extends Api>(
	model: Model<TApi>,
	instructions: string,
): ResponsesInputItem[] {
	const text = instructions.trim();
	return text ? [{ role: model.reasoning ? "developer" : "system", content: sanitizeSurrogates(text) }] : [];
}
