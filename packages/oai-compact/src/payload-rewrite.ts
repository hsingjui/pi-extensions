...
function startsWithEquivalentItems(actual: readonly unknown[], expectedPrefix: readonly unknown[]): boolean {
	if (actual.length < expectedPrefix.length) {
		return false;
	}

	for (let index = 0; index < expectedPrefix.length; index++) {
		if (!areEquivalentValues(actual[index], expectedPrefix[index])) {
			return false;
		}
	}
...
export function extractFreshAuthoritativePreamble(
	...
	return {
		...
		leadingInput: payload.input
			...
			.map((item: unknown) => cloneResponsesInputMessageItem(item as ResponsesInputMessageItem)),
...
function buildNativeReplaySegmentsInternal<TApi extends Api>(args: {
	...
	const exactReplayMatch = areEquivalentValues(args.payload.input, originalPiReplayInput);
	const replayPlusCurrentSuffix = !exactReplayMatch && startsWithEquivalentItems(args.payload.input, originalPiReplayInput);
	if (!exactReplayMatch && !replayPlusCurrentSuffix) {
