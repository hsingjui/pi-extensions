...
function readOrientationFromTiff(bytes: Uint8Array, tiffStart: number): number {
	...
	const read16 = (pos: number): number => (le ? bytes[pos] | (bytes[pos + 1] << 8) : (bytes[pos] << 8) | bytes[pos + 1]);
...
function findJpegTiffOffset(bytes: Uint8Array): number {
	...
	while (offset < bytes.length - 1) {
		...
		if (marker === 0xe1) {
			...
			if (segmentStart + 6 > bytes.length || !hasExifHeader(bytes, segmentStart)) return -1;
...
function getExifOrientation(bytes: Uint8Array): number {
	let tiffOffset = -1;
	if (bytes.length >= 2 && bytes[0] === 0xff && bytes[1] === 0xd8) tiffOffset = findJpegTiffOffset(bytes);
	else if (bytes.length >= 12 && bytes[0] === 0x52 && bytes[1] === 0x49 && bytes[2] === 0x46 && bytes[3] === 0x46 && bytes[8] === 0x57 && bytes[9] === 0x45 && bytes[10] === 0x42 && bytes[11] === 0x50) tiffOffset = findWebpTiffOffset(bytes);
...
export function applyExifOrientation(photon: Photon, image: PhotonImageType, originalBytes: Uint8Array): PhotonImageType {
